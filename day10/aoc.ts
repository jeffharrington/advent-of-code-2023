import { Advent, f, neighbors } from "advent";
...
compute(2, async (input) => {
	...
	let visited = Set<string>(`${i},${j}`);
	let loop = Array.from(new Array(data.length * 2), () => new Array(data[0].length * 2).fill(false));
	...
	while (queue.length > 0) {
		...
		for (const pt of neighbors([i, j], false)) {
			const [ii, jj] = pt;
